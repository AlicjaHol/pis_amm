...
Podobnie jak w przypadku prostej metody średniej ruchomej - im większy parametr $\eta$, tym bardziej wygładzony wykres, ale i mniejsza dokładność. Jednakże, dokładnośc jest i tak większa niż w przypadku prostej metody ruchomej średniej.
...
ARIMA JEST JESZCZE DO POPRAWKI!!!!!!!
...
![Kawa ze Starbucks](sbux_logo.jpg)

Kawa ze Starbucks jest znana z tego, że pojawiła się jako błąd w jednym z odcinków Gry o tron.

![Starbucks jest tak popularny, że piją go nawet w fantasy, które dzieje się w średniowieczu!](gra-o-tron-starbucks.jpg)
...
Podobno kawa w Starbucksie jest droższa niż paliwo w Polsce!!!!
![ach ta akcyza](Starbucks-Gas.jpg)
...
```{r wykładnicza 0.2 SBUX}
wykladnicza(sbux, 0.2, "red")
```

```{r wykładnicza 0.5 SBUX}
wykladnicza(sbux, 0.5, "green")

```

```{r wykładnicza 0.7 SBUX}
wykladnicza(sbux, 0.7, "blue")
```

```{r wykładnicza 0.9 SBUX}
