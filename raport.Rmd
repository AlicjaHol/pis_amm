...
```{r}
par(mfrow = c(1,3))
plot(density(mcd_arima$residuals), main = "Wykres gęstości rozkładu reszt",
     ylab = "Gęstość")
...
Ponieważ badany szereg czasowy nie wykazuje sezonowości, nie możemy zatem przeprowadzić predykcji metodą Holta-Wintersa. Zamiast tego użyjemy metody Holta, korzystając z funkcji `Holt` biblioteki `aTSA`.

```{r}
library(aTSA)
plot(Holt(mcd, plot = F)$estimate, main = "Predykcja" , xlab = "Oszacowania", ylab = "Czas",
     type = "l", lwd = 2, col = "maroon3")
plot(mcd, type = "l")
```
